...
import { createGalleryAlbum, type ActionState } from "../actions";
...
export default function NewGalleryAlbumPage() {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    if (e.target.files) {
      ...
      const combinedFiles = [...selectedFiles, ...newFiles].slice(0, 20); // Max 20 photos per album
    ...
    <div className="p-5 sm:p-7 lg:p-8 max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        >
          <div className="p-6 sm:p-8 flex flex-col gap-5">
            ...
            >
              <input
                ...
                name="title"
                required
                placeholder={t.adminGalleryEdit.fieldTitlePlaceholder}
                className={inputCls}
              />
            </Field>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              >
                <input
                  type="text"
                  name="event_name"
                  placeholder={t.adminGalleryEdit.fieldEventPlaceholder}
                  className={inputCls}
                />
              </Field>
              <Field
              ...
              >
                <input type="date" name="taken_at" className={inputCls} />
              </Field>
            </div>
          </div>
          ...
          <div className="p-6 sm:p-8 flex flex-col gap-5">
            <SectionHeading
              title={`${t.adminGalleryEdit.sectionImages} (${selectedFiles.length}/20)`}
            />

            <Field
              label={t.adminGalleryEdit.fieldNewImages}
              required
              hint={t.adminGalleryEdit.fieldNewImagesHint}
            >
              <div
                onClick={() => {
                  if (selectedFiles.length < 20) fileInputRef.current?.click();
                }}
            ...
            {selectedFiles.length > 0 && (
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 mt-2">
                {previewUrls.map((url, i) => (
                  <div
                    key={i}
                    className="relative group rounded-xl overflow-hidden ring-1 ring-gray-200 bg-gray-50 aspect-square"
                  >
                    <img
                      src={url}
                      alt="Preview"
                    ...
                    />
                    {i === 0 && (
                      <div className="absolute top-0 left-0 bg-[#c9a84c] text-[#0a1628] text-[10px] font-bold px-2 py-1 rounded-br-lg">
                        {t.adminGalleryEdit.badgeCover}
                      </div>
                    )}
                    <button
                      type="button"
              ...
              >
                {isPending ? (
                  <>
                    ...
                    {t.adminGalleryEdit.savingBtn}
                  </>
                ) : (
                  <>
                    <Check className="w-4 h-4" />{" "}
                    {t.adminGalleryEdit.saveBtn.replace("✓ ", "")}
                  </>
                )}
              </button>
            </div>
